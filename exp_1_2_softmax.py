...
for i in range(1, 10):
        ...
        data_path = r"{0}\exp_1_1\softmax\rule_{1}\training_data_entropy_x_y.txt".format(dir_path, i)
        data = np.loadtxt(data_path, dtype=float, delimiter=" ")
        majority_amount = int(data.shape[0] * 0.95)
        majority_x = data[:majority_amount, 1:-2]
        ...
        major_data_x_benign_part = majority_x[np.where(majority_y == 0)[0]]
