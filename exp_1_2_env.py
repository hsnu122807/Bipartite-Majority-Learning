...
for i in range(1, 10):
        ...
        data_path = r"{0}\exp_1_1\env\rule_{1}\training_data_residual_predict_output_desire_output_desire_input.txt".format(dir_path, i)
        data = np.loadtxt(data_path, dtype=float, delimiter=" ")
        majority_amount = int(data.shape[0] * 0.95)
        majority_x = data[:majority_amount, 3:]
        majority_y = data[:majority_amount, 2].reshape((-1, 1))
