...
for i in range(3):
    ...
    for kernel in kernel_arr:
        target_path = r"{0}\svm_{1}\{2}".format(path, kernel, path_arr[i])
        method = 'svm_{0}'.format(kernel)
        h_n = 0
        with open(target_path + r'\_two_class_training_detail.txt') as f1:
            while 1:
                line = f1.readline()
...
kernel_arr = ['linear', 'rbf', 'poly']
for kernel in kernel_arr:
    file = open(new_path + r"\svm_{0}_train_test_analyze.txt".format(kernel), 'w')
    file.writelines('Sampling Method, Train FP, Train FN, Test FP, Test FN\n')
    for i in range(3):
        target_path = r"{0}\svm_{1}\{2}".format(path, kernel, path_arr[i])
        ...
        file.writelines('{0}, {1}/{2}, {3}/{4}, {5}/{6}, {7}/{8}\n'.format(path_arr[i], one, two, three, four, five, six, seven, eight))
    file.close()
