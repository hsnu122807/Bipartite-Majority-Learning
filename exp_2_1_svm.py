...
for owl in range(1, 10):
    ...
    for kernel in kernel_arr:
        ...
        if not os.path.exists(new_path):
            os.makedirs(new_path)
        # 從事先抽好的train中拿資料
        mal_training_data = np.loadtxt(r"exp_2_1\data\rule_{0}\mal_training_data.txt".format(owl_rule), dtype=float, delimiter=" ")
        benign_training_data = np.loadtxt(r"exp_2_1\data\rule_{0}\benign_training_data.txt".format(owl_rule), dtype=float, delimiter=" ")
        ...
        mal_testing_data = np.loadtxt(r"exp_2_1\data\rule_{0}\mal_testing_data.txt".format(owl_rule), dtype=float, delimiter=" ")
        benign_testing_data = np.loadtxt(r"exp_2_1\data\rule_{0}\benign_testing_data.txt".format(owl_rule), dtype=float, delimiter=" ")
        mal_y = np.array([-1], dtype=float)
        benign_y = np.array([1], dtype=float)
